...
description: Codex adapter for implementing a validated mivia-agent-plan/v1 DAG with scoped edits, verifiers, mutation proofs, hooks, and audit loops.
triggers:
  - implement this plan
  - implement the plan
  - execute dag plan
...
- Run node verifiers, mutation proof, and required audit/review loops.
- End implementer reports with `PlanArtifact: .ai/plans/<id>.plan.json`.
