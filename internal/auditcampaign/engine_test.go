...
func TestEngineRejectsSelfConfirm(t *testing.T) {
	...
	eng := &Engine{
		...
		InteractiveContinuous: true,
		SelfConfirm:           true,
		Audit:                 func(context.Context, Phase, int, Evidence) (Evidence, error) { return Evidence{}, nil },
	}
...
func TestEnginePassesAuditEvidenceToConfirmAndFix(t *testing.T) {
	dir := t.TempDir()
	var confirmPrior, fixPrior Evidence
	eng := &Engine{
		Campaign: testCampaign(),
		Store:    NewStore(dir, "c-prior", "o"),
		Audit: func(ctx context.Context, phase Phase, cycle int, prior Evidence) (Evidence, error) {
			if cycle > 1 {
				return Evidence{Schema: EvidenceSchema, CampaignRun: "r", BaselineHead: "h2", Cycle: cycle}, nil
			}
			return Evidence{
	...
	if confirmPrior.FindingFingerprint != "fp-wire" {
		t.Fatalf("confirm prior = %+v, want audit fingerprint fp-wire", confirmPrior)
	}
	if fixPrior.FindingFingerprint != "fp-wire" || fixPrior.Disposition != DispositionConfirmed {
		t.Fatalf("fix prior = %+v, want confirmed fp-wire", fixPrior)
	}
	if res.Commits != 1 {
